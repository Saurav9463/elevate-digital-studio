...
export function ProjectCard({ project, index = 0 }: { project: ProjectCardData; index?: number }) {
  const Wrapper = project.live_url
      ...
      <Wrapper>
        ...
        <div className="flex flex-1 flex-col gap-3 p-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <ArrowUpRight className={`mt-1 h-4 w-4 transition-transform duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 ${project.live_url ? "text-primary" : "text-muted-foreground group-hover:text-accent"}`} />
          </div>
          ...
          {project.live_url && (
            <p className="text-[11px] font-medium text-primary mt-1">View live site →</p>
          )}
        </div>
      </Wrapper>
    </motion.article>
